...
/**
 * Compressed SVG equity chart — fits the entire date range into the
...
export default function CustomEquityChart({ strategies, pointsByStrategy, startingCapital = 100000 }) {
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      ...
      setSize({ w: Math.max(200, Math.floor(r.width)), h: Math.max(120, Math.floor(r.height)) });
    });
    ro.observe(el);
    return () => ro.disconnect();
  }, []);

  const padding = { l: 56, r: 12, t: 16, b: 24 };
  const innerW = Math.max(1, size.w - padding.l - padding.r);
  const innerH = Math.max(1, size.h - padding.t - padding.b);

  // Aggregate domain across all strategies.
  const { tMin, tMax, vMin, vMax, hasData } = useMemo(() => {
    ...
    if (!hasData) { return { tMin: 0, tMax: 1, vMin: 99, vMax: 101, hasData: false }; }
    if (tMin === tMax) tMax = tMin + 1;
    // Pad value range for headroom and always include the 100 baseline.
    vMin = Math.min(vMin, 100);
    vMax = Math.max(vMax, 100);
    const pad = (vMax - vMin) * 0.08 || 1;
    vMin -= pad; vMax += pad;
    return { tMin, tMax, vMin, vMax, hasData };
  }, [strategies, pointsByStrategy]);

  const xOf = (t) => padding.l + ((t - tMin) / (tMax - tMin)) * innerW;
  const yOf = (v) => padding.t + (1 - (v - vMin) / (vMax - vMin)) * innerH;
  ...
  // Y-axis ticks (% of starting capital) — 5 ticks.
  const yTicks = useMemo(() => {
    const n = 5;
    const out = [];
    for (let i = 0; i <= n; i++) {
      const v = vMin + ((vMax - vMin) * i) / n;
      out.push({ v, y: yOf(v), dollars: (v / 100) * startingCapital });
    }
    return out;
  }, [vMin, vMax, innerH, padding.t, startingCapital]);
  ...
  return (
    <div ref={wrapRef} className="relative w-full h-full">
      <div className="absolute top-2 left-3 text-[10px] uppercase tracking-widest text-muted z-10 pointer-events-none">
        Equity (% of {fmtUsd(startingCapital)})
      </div>
      ...
      >
        ...
        {xTicks.map((tk, i) => (
          <g key={i}>
            <line
              x1={tk.x} x2={tk.x}
              y1={size.h - padding.b} y2={size.h - padding.b + 4}
        ...
        {hover && hasData && (
          <g pointerEvents="none">
            ...
            {strategies.map((s) => {
              ...
              if (!pt) return null;
              const cx = xOf(pt.time);
              const cy = yOf(pt.value);
              return <circle key={s.id} cx={cx} cy={cy} r={3} fill={s.color} />;
            })}
          </g>
        )}
      </svg>
